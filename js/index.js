    ...
    var Player = {
        ...
        init: function () {
            ...
            try {
                ...
                request.open('GET', '/getSongName', true)
                request.responseType = 'text';

                request.onload = function() {
                    that.loadTrack(request.response);
                };
        ...
        loadTrack: function (song) {
            ...
            var request = new XMLHttpRequest();
            var name = song.split('-');
            document.querySelector('.song .artist').textContent = name[0];
            document.querySelector('.song .name').textContent = name[1];

            request.open('POST', '/getSongFile', true);
            ...
            request.onload = function() {
                that.context.decodeAudioData(request.response, function(buffer) {
                    ...
                    that.source.connect(that.context.destination);       // connect source to context's destination (the speakers)
